...
enemies populate as they lock, not on hover. **Ally** hovers are visible, and in
a ranked queue (solo/duo or flex) they feed the ranked-draft panel below.

### Ranked draft assistant

In a ranked champ select the Build page adds a **Ranked Draft** panel driven by
per-elo champion stats (`src/data/rankedChampStats.json`, generated by
`npm run aggregate:ranked` — see "Ranked champion stats" below):

- **Ban targets** — the most-OP champions in *your* elo bucket (smoothed win
  rate weighted by pick+ban presence), skipping champions already banned or
  hovered by your team. Your rank comes from the LCU; if no dataset exists for
  your bucket yet, the nearest available one is used and labeled.
- **Pick suggestions** — for your assigned role, ranked by elo win rate and
  bumped for what your team's hovers are missing (frontline, AP/AD balance,
  hard CC) and what the enemy comp rewards. Click one to load its build.

`?mockcs=1` previews the panel too (mock: ranked queue, Gold II, an all-carry
ally comp).
...
## Ranked champion stats (`scripts/aggregate-ranked-stats.mjs`)

Feeds the ranked-draft panel: per-elo champion pick/win/ban rates plus per-role
win rates, from ranked solo/duo (queue 420) Match-V5 data. Elo buckets:
`iron_bronze`, `silver_gold`, `plat_emerald`, `diamond`, `master_plus` — the
non-apex ones sample a page of the tier's division-II ladder, the apex one uses
the master/GM/challenger leagues (the same pool the build aggregator samples).
...
records which bucket sampled which match so Silver games never pollute the
